...
for iG = 1:size(t2g,1)
    ...
    % outfun = @(x, optimValues, state) outfunwrapper(x, optimValues, ...
    %     state, [folder_name filesep pad(num2str(SimTime),7,"left",'0')]);
...
function stop = outfunwrapper(x, optimValues, state, name)
    stop = false; % Don't stop the optimization

    % Specify the CSV file name
    filename = [name '.csv'];

    % Handle different states of the optimization
    switch state
        case 'init'
            % Write header to the CSV file during initialization
            fid = fopen(filename, 'w');
            fprintf(fid, 'Iteration, Cost, %s\n', sprintf('x%d,', 1:length(x)));
            fclose(fid);
            
        case 'iter'
            % Write the current iteration data to the file
            fid = fopen(filename, 'a');
            fprintf(fid, '%d, %f, %s\n', optimValues.iteration, optimValues.fval, sprintf('%f,', x));
            fclose(fid);
            
        case 'done'
            % Finalization code, if any, can go here
    end
end