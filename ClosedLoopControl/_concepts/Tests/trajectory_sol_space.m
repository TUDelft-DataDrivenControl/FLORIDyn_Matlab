...
%% Bezier curve approach with 1 dof
figure
subplot(2,2,1)
hold on
...
%     o2 = rand(nBranches,1); % g_so
    fB = factor(nBranches);
    [o1,o2] = meshgrid(linspace(0,1,fB(end)),linspace(0,1,prod(fB(1:end-1)))); % t_so% g_so
...
for nI = 1:nIterations
    o1 = linspace(0,1,nBranches); % g_so
...
%exportgraphics(gcf,'sol_space_tranjectories.pdf','ContentType','vector')