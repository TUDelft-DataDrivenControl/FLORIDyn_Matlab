...
for iS = 1:EnKF.Sim.Sections
    ...
    if EnKF.Vel.Correct
        ...
        K_pu = EnKF_CalcKalmanGain('cxy_cyy_cee_loc', ...
                'Cxy',C_xy_Vel,...          % State-to-Output cov. matrix  % FOR ALL ENSEMBLES
                'Cyy',C_yy_Vel,...          % Output covariance matrix     % FOR ALL ENSEMBLES
                'Cee',EnKF.Vel.C_ee_Vel,... % Measurement covariance matrix% FOR ALL ENSEMBLES
                'Loc',LocCov);              % Localization                 % FOR ALL ENSEMBLES
    end
    if EnKF.Dir.Correct
        C_xx_Dir = EnKF_CalcStateCov(EnKF.States.Dir,EnKF.nE,T.StartI);

        K_phi = EnKF_CalcKalmanGain('c_cxx_cee_loc', ...
    ...
    for iE = 1:EnKF.nE
        ...
        if and(sum([EnKF.Vel.loc, EnKF.Dir.loc, EnKF.TI.loc])>0,...
                ~projectOntoTrueState)
            % In case localization is needed on an ensemble basis, the
            % distance between the OPs is calculated.
            OPs_tmp = ...
...
%               'C', EnKF.weightedInteractionVel{iE});
            ...
            % === K Matrix calculation for each ensemble ================ %
%             if EnKF.Vel.loc
%                 % create localization covariance and multiply with state
%                 % error covariance matrix
%                 LocCov = GaspariAndCohn1999(EnKF.Vel.cutOffLength,distOPs);
%                 
%                 % Kalman gain matrix based on
%                 %   Cxx State covariance matrix
%                 %   Cee Measurement covariance matrix
%                 %   Loc Locatization covariance
%                 %   C   Ensemble individual velocity output
%                 K = EnKF_CalcKalmanGain('c_cxx_cee_loc', ...
%                     'Cxx',C_xx_Vel,...
%                     'Cee',EnKF.Vel.C_ee_Vel,...
%                     'Loc',LocCov,...
%                     'C',EnKF.weightedInteractionVel{iE});
%             else
%                 % No localization
...
%             end
            ...
            % === Correction based on wind speed ======================== %
            % === Ensemble Individual =================================== %
...
%                 K_phi * (d_Dir_j - EnKF.States.Dir(T.StartI,iE));
...
%               [EnKF_Vis, err(iS,:)] = plotK_FlowField(EnKF_Vis, EnKF, T, Wind, Sim, ...
...
t = toc(tStart);
timeErr = (1:length(err))*EnKF.Sim.SecDur;
writematrix([timeErr', err],['Error_CDC_' num2str(EnKF.nE) '.csv'])
