...
[UKF, Wind] = EnKF_PreProcessing(UKF, T, Wind, Sim);
...
for iS = 1:UKF.Sim.Sections
    ...
    for iE = 1:UKF.nE
        ...
        % === Collective K
        UKF.States.Vel(:,iE) = UKF.States.Vel(:,iE) + ...
            K_pu * (d_P - UKF.Output.Pow(:,iE) + ...
            (randn(1,T.nT)*UKF.Output.C_ee_Pow_Chol)');

        % ============ Correct wind direction =============================
        d_Dir_j = d(:,2) + ...
                (randn(1,T.nT)*UKF.Dir.C_ee_Dir_Chol)';
        % === Collective ============================================ %
        UKF.States.Dir(:,iE) = UKF.States.Dir(:,iE) + ...
            K_phi * (d_Dir_j - UKF.States.Dir(T.StartI,iE));
    end
    
    %% Reseed
    % Plan is to have some faster ensembles, some slower ones, some in a 
    % higher wind dir, some in a lower. The distribution function should
    % not get lost
    ...
    % Determine range based on cumulative distribution function (inverted)
    nSeed = sqrt(UKF.nE);
    range_vel = sqrt(2)*stdVel*erfinv(2*linspace(.2,.8,nSeed)-1)+meaVel;
    range_dir = sqrt(2)*stdDir*erfinv(2*linspace(.2,.8,nSeed)-1)+meaDir;
    for iSeed_u = 1:nSeed
        for iSeed_phi = 1:nSeed
            UKF.States.Vel(:,(iSeed_u-1)*nSeed+iSeed_phi) = range_vel(:,iSeed_u);
            UKF.States.Dir(:,(iSeed_u-1)*nSeed+iSeed_phi) = range_dir(:,iSeed_phi);
        end
    end
